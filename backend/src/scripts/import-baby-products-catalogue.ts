...
const DEFAULT_PATH = path.join(homedir(), "Downloads", "baby_products_catalog_1000.json");
const cataloguePath = path.resolve(process.argv[2] ?? process.env.BABY_PRODUCTS_CATALOG_PATH ?? DEFAULT_PATH);
...
const products = rows.map((row) => {
  if (row.section !== "Baby Care") throw new Error(`Unsupported section '${row.section}' for ${row.sku}.`);
  ...
  return {
    ...
    specifications: new Map([
      ...
      ["imageStatus", "Related Baby Care image used because the supplied catalogue image is a grey placeholder"],
    ...
    tags: ["Baby Care", row.category, row.brand, row.name, quantity, row.ageGroup ?? "", "catalogue"].filter(Boolean),
    isActive: true,
  };
});
...
try {
  ...
  console.log(`Baby Products import: ${result.upsertedCount} inserted, ${result.modifiedCount} updated.`);
  console.log(`${categories.length} Baby Care subcategories; ${products.length} active Baby Care catalogue products.`);
